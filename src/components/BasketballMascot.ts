...
function teamBadge(teamName: string, teamColor: number) {
  ...
  if (context) {
    ...
    context.fillText(teamName.split(' ').map((word) => word[0]).join('').slice(0, 3), 128, 140);
    context.fillStyle = '#ffffff';
    context.font = '900 22px Arial';
    context.fillText(teamName.toUpperCase().slice(0, 18), 128, 278);
  }
  return new THREE.CanvasTexture(canvas);
}

export function createBasketballMascot(scene: THREE.Scene, teamName: string, teamColor: number): BasketballMascot {
  const group = new THREE.Group();
  const orange = new THREE.MeshStandardMaterial({ color: teamColor, roughness: .7 });
  ...
  const head = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xe7c07b, roughness: .75 }),
  );
  ...
  const badge = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ map: teamBadge(teamName, teamColor), side: THREE.DoubleSide }),
  );
